...
if (attack_key && !is_attacking && !is_reloading && current_ammo > 0) {
    ...
    fireball_spawned = false; // Reset fireball spawn flag
}
...
if (is_attacking) {
    ...
    // Spawn fireball near the end of attack animation (10 frames before end)
    if (attack_timer == 10 && !fireball_spawned) {
        fireball_spawned = true;
        
        // Calculate spawn position 25 pixels in front of player
        var spawn_x = x + (facing_direction * 25);
        var spawn_y = y;
        
        // Create fireball projectile at spawn position
        var fireball = instance_create_layer(spawn_x, spawn_y, "Instances", obj_blue_fireball);
        
        // Set fireball direction based on facing direction
        if (facing_direction == 1) {
            fireball.direction = 0; // Right
        } else {
            fireball.direction = 180; // Left
        }
    }
